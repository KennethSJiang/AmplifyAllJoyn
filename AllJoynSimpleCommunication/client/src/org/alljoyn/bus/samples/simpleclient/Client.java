...
//            });
        ...
        Button startBttn = (Button) findViewById(R.id.start);
        startBttn.setOnClickListener(new View.OnClickListener() {
			
			@Override
			public void onClick(View v) {
				int counter = 0;
				while(counter < TOTAL_SAMPLE_POINTS){
                	long startTime = System.currentTimeMillis();
    				Message msg = mBusHandler.obtainMessage(BusHandler.PING, String.valueOf(startTime) );
    				mBusHandler.sendMessage(msg);
					counter++;
				}
			}
            ...
            case (JOIN_SESSION): {
                ...
                Status status = mBus.joinSession((String) msg.obj, contactPort, sessionId, sessionOpts, new SessionListener() {
                    ...
                    public void sessionLost(int sessionId) {
                        mIsConnected = false;
                        logInfo(String.format("MyBusListener.sessionLost(sessionId = %d)", sessionId));
                        mHandler.sendEmptyMessage(MESSAGE_START_PROGRESS_DIALOG);
                    }
                });
